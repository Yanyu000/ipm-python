...
\newcommand{\Try}[2]{\textbf{try}\;#2}
\newcommand{\Catch}[2]{\textbf{catch} #1\;#2}

% ---------------------------------------------------------------------------
% Title
% ---------------------------------------------------------------------------
\title{
  Preliminary CPU Reference Study for GPU-Oriented \\
  Inexact IPM Newton Solves
}
...
\begin{abstract}
  This manuscript reports a CPU reference study for GPU-oriented inexact
  Newton solves inside primal-dual interior point methods for linear
  programming.  The target linear system is the normal equation
  $\bM_k \bDelta\by_k = \br_k$, with
  $\bM_k = \bA\bD_k^2\bA^T$.  We distinguish complete blocked Cholesky,
  which is a direct solver only when off-diagonal updates and Schur
  complements are included, from diagonal-block Cholesky, which is only a
  block-Jacobi preconditioner and must be paired with CG or refinement.
  The CPU prototype compares FP64 dense Cholesky, blocked dense Cholesky,
  simulated FP32 Cholesky, block-Jacobi PCG, and adaptive residual
  refinement.  Experiments on small dense random instances up to
  $m=200$, $n=600$ show that fixed-accuracy low-precision and
  block-Jacobi solvers can fail as the normal equations become
  ill-conditioned, while adaptive refinement can match the exact CPU
  baseline's convergence on these tests.  These results are preliminary:
  they do not demonstrate GPU speedup or HIPO integration, but they define
  the frozen-Newton benchmarks needed to evaluate a future GPU backend
  under the same residual tolerance.
\end{abstract}
...
The present implementation is a CPU-only Python prototype.  It is designed
to test linear-solver accuracy, stability, and failure modes before any
HIPO integration.  References to GPU or multi-GPU execution in this draft
therefore describe the intended backend and benchmark target, not a
measured speedup.  A GPU acceleration claim would require an actual GPU
benchmark, preferably on frozen Newton systems exported from HIPO, with
the same residual tolerance and an explicit policy for matrix formation
and host/device transfer timing.
...
We study this bridge in a CPU reference prototype for a future
\emph{GPU-oriented mixed-precision inexact IPM Newton solver}.  The target
backend combines three ideas.  First, the normal-equations matrix $\bM_k$
may be partitioned into $p$ diagonal blocks and used to build a
block-diagonal preconditioner
$\bP_k = \blkdiag(\bM_{11},\dots,\bM_{pp})$.  This CPU prototype implements
that strategy in \texttt{\_build\_block\_diagonal\_preconditioner} in
\texttt{linear\_solver.py}, which slices $\bM_k$ into \texttt{num\_blocks}
contiguous diagonal blocks and factors each block via Cholesky.  Second,
the Newton step can be computed by preconditioned conjugate gradient or
mixed-precision iterative refinement, reusing the block factors to correct
the solution.  Third, and most critically, the target accuracy for the
linear solve is adaptively scheduled as a piecewise function of $\mu_k$,
implemented by \texttt{\_mu\_to\_eta\_target} in
...
provides a conservative CPU fallback when the low-precision factorization
...
The current prototype makes the following limited contributions.  It
summarizes the standard inexact-IPM residual condition
$\eta_k \le \bar{c}\mu_k^\alpha$ and uses it as a design rule for
linear-solve tolerance scheduling (Section~\ref{sec:theory}).  It
implements CPU diagnostic solvers that separate complete blocked Cholesky
with Schur-complement updates from diagonal-block Cholesky used only as a
block-Jacobi preconditioner (Section~\ref{sec:precond}).  It implements a
$\mu$-driven adaptive refinement solver with FP64 residual checks and FP64
fallback (Section~\ref{sec:adapt}, Algorithm~\ref{alg:adapt}).  Finally,
it reports CPU experiments on dense LP instances up to $m=200$, $n=600$
showing that fixed-accuracy low-precision and block-Jacobi solvers can
fail on these tests, while adaptive refinement tracks the exact CPU
baseline in iteration count and solution quality
...
\begin{theorem}[Informal residual condition for inexact IPM]
  \label{th:conv}
  Let Assumptions~\ref{as:feas} and~\ref{as:rank} hold, together with the
  usual regularity assumptions in inexact primal-dual IPM analyses.  Suppose
  that at each iteration $k$ of the Mehrotra predictor-corrector IPM, the
  linear solve satisfies
  \begin{equation}
    \boxed{\eta_k \le \bar{c}\,\mu_k^\alpha},
    \label{eq:eta-condition}
  \end{equation}
  for constants $\bar{c} > 0$ and $\alpha \in (0,1]$ independent of $k$.
  Then standard inexact-IPM analyses imply that, for sufficiently small
  constants, there exists a constant $\delta > 0$ such that
  \begin{equation}
...
\begin{proof}[Proof sketch]
  This is a summary of the standard proof strategy rather than a new
  theorem established by the CPU prototype.  The proof adapts the standard
  analysis of polynomial IPM convergence
...
throughput ratio can be much larger, the working hypothesis is that early
and mid-iteration low-precision factorizations may compensate for late
refinement overhead.  This remains to be measured on an actual GPU
benchmark and is not established by the CPU data.
...
\begin{figure}[p]
  \centering
  \includegraphics[width=0.95\linewidth]{convergence_mu.pdf}
  \caption{Duality-measure trajectories for the CPU IPM benchmark.}
  \label{fig:conv}
\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.95\linewidth]{condition_number.pdf}
  \caption{Condition-number trajectories for the normal equations.}
  \label{fig:cond}
\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.95\linewidth]{eta_residual.pdf}
  \caption{Linear-solve relative residuals measured in FP64.}
  \label{fig:eta}
\end{figure}

\begin{figure}[p]
  \centering
  \includegraphics[width=0.85\linewidth]{total_time.pdf}
  \caption{CPU wall-clock timings for the full IPM benchmark.}
  \label{fig:time}
\end{figure}
...
We have presented a CPU reference prototype and benchmark plan for a
GPU-oriented mixed-precision inexact IPM Newton solver.  The prototype
separates three ideas that should not be conflated: complete blocked
Cholesky with Schur-complement updates is a direct solver; diagonal-block
Cholesky is only a block-Jacobi preconditioner; and adaptive refinement is
an accuracy-recovery mechanism driven by the IPM duality measure.  The
current implementation performs residual checks in FP64 and falls back to
FP64 Cholesky when the simulated low-precision solve cannot meet the
target.
...
is to export or intercept frozen Newton systems from HIPO and rerun the
linear-solver benchmark under identical residual tolerances.  Only after
that step should CUDA/CuPy or native CUDA kernels be used to test whether
low-precision dense factorizations deliver a net speedup once matrix
formation, transfer, and fallback costs are counted.  Multi-GPU
distributed Cholesky with off-diagonal coupling communicated via MPI or
NCCL would extend the block-diagonal preconditioner to a full distributed
direct solver where the application warrants it.  Extending the framework
to augmented KKT systems would mitigate the normal-equations conditioning
issue analyzed in Section~\ref{sec:prelim}, at the cost of working with an
indefinite system.  Finally, the analogy with quantum interior point
methods~\cite{Kerenidis2020} remains useful as an error-management lens,
but the present evidence is classical and CPU-only.
